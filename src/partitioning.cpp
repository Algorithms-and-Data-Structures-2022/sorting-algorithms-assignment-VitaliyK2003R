...
namespace assignment {
  ...
  int median_of_three(const std::vector<int>& arr, int start, int stop) {
    ...
    if (((arr[start] <= arr[middle])&&(arr[middle] <= arr[stop]))||((arr[stop] <= arr[middle])&&(arr[middle] <= arr[start]))) {
      return middle;
    }
    if (((arr[start] <= arr[stop])&&(arr[stop] <= arr[middle]))||((arr[middle] <= arr[stop])&&(arr[stop] <= arr[start]))) {
      return stop;
    }
    if (((arr[middle] <= arr[start])&&(arr[start] <= arr[stop]))||((arr[stop] <= arr[start])&&(arr[start] <= arr[middle]))) {
      return start;
    }
  }
